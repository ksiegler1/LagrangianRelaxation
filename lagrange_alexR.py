...
def update_lambdas(dem_df, alpha, curr_ub, blb):
    t_n = (alpha * (curr_ub - blb)) / (sum(dem_df['const_output']**2)+0.01)
...
def run(max_p, num_nodes, D_c, demand_file, distance_file):
    ...
    for p in range(3, max_p):
        temp_pct, temp_p, temp_time, temp_gap = main(p, num_nodes=num_nodes, D_c=D_c,
                                                     demand_file=demand_file, distance_file=distance_file)
        p_vals.append(temp_p)
        pct_vals.append(temp_pct)
        time_vals.append(temp_time)
        gap_vals.append(temp_gap)

    return p_vals, pct_vals, time_vals, gap_vals

if __name__ == '__main__':

    D_c = 200

    p_vals49, pct_vals49, time_vals49, gap_vals49 = run(max_p=20, num_nodes=49, D_c=D_c,
                                                        demand_file='49 NodeDemandData.txt',
                                                        distance_file='49Node.txt')

    p_vals88, pct_vals88, time_vals88, gap_vals88 = run(max_p=20, num_nodes=88, D_c=D_c,
                                                        demand_file='88NodeDemandData.txt',
                                                        distance_file='88Node.txt')

    p_vals150, pct_vals150, time_vals150, gap_vals150 = run(max_p=20, num_nodes=150, D_c=D_c,
                                                            demand_file='150 NodeDemandData.txt',
                                                            distance_file='150 Node Distance Data.txt')

    # save the data to file
    run_data = pd.DataFrame({'p_vals49': p_vals49, 'pct_vals49': pct_vals49, 'time_vals49': time_vals49, 'gap_vals49': gap_vals49,
                  'p_vals88': p_vals88, 'pct_vals88': pct_vals88, 'time_vals88': time_vals88, 'gap_vals88': gap_vals88,
                  'p_vals150': p_vals150, 'pct_vals150': pct_vals150, 'time_vals150': time_vals150, 'gap_vals150': gap_vals150})

    run_data.to_csv('run_data_' + str(D_c)+'.csv', index=False)

    # plot percent coverage vs number of facilities
    plt.plot(p_vals49, pct_vals49, color='red', label='49')
    plt.plot(p_vals88, pct_vals88, color='blue', label='88')
    plt.plot(p_vals150, pct_vals150, color='green', label='150')
    plt.xlabel('Number of Facilities', size=22)
    plt.ylabel('Percent Coverage', size=22)
